// SCL-HUB USER API - The Final, Corrected, SDK-Compliant Server
...
// --- MAIN API ENDPOINTS (Rewritten with CORRECT SDK methods) ---
...
// 4. Endpoint to GET ONE specific cluster
...
app.post('/api/create-cluster', async (req, res) => {
    try {
        ...
        // --> THE 110% FIX: Using the correct SDK method: `webflow.items.createItem`
        const newItem = await webflow.items.createItem({ collectionId: COLLECTION_ID, fieldData });
        
        // The SDK's createItem method does not return the full item, so we fetch it.
        const createdItem = await webflow.items.get({ collectionId: COLLECTION_ID, itemId: newItem.id });

        // Now, publish and add the item ID
        await webflow.items.patchItem({
            collectionId: COLLECTION_ID,
            itemId: createdItem.id,
            fieldData: { 'webflow-item-id': createdItem.id, '_archived': false, '_draft': false }
        });
...
app.patch('/api/update-cluster', async (req, res) => {
    try {
        ...
        // --> THE 110% FIX: Using the correct SDK method: `webflow.items.patchItem`
...
app.delete('/api/delete-cluster', async (req, res) => {
    try {
        ...
        // --> THE 110% FIX: Using the correct SDK method: `webflow.items.removeItem`
...
app.post('/api/upload-image', upload.single('file'), async (req, res) => {
    try {
        ...
        // --> THE 110% FIX: Using the correct SDK method: `webflow.assets.uploadAsset`
...
// --- Full Auth Endpoints (for completeness) ---
app.get('/', (req, res) => { const installUrl = WebflowClient.authorizeURL({ clientId: WEBFLOW_CLIENT_ID, scope: "cms:read cms:write assets:write" }); res.redirect(installUrl); });
app.get('/auth/callback', async (req, res) => { try { const { code } = req.query; if (!code) return res.status(400).send("Authorization code is missing."); const token = await WebflowClient.getAccessToken({ clientId: WEBFLOW_CLIENT_ID, clientSecret: WEBFLOW_CLIENT_SECRET, code: code }); webflowAccessToken = token; console.log("SUCCESS: Webflow Access Token received and stored."); res.send("<h1>Authentication Successful!</h1><p>Your SCL-HUB application is now connected. You can close this window.</p>"); } catch (error) { console.error("OAuth Callback Error:", error); res.status(500).send("An error occurred during authentication."); } });